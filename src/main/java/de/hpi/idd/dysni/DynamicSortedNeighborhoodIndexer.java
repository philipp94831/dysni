...
public class DynamicSortedNeighborhoodIndexer<T extends HasId<String>> {
	...
	private final List<Tuple<KeyWrapperFactory<T, String>, SimilarityAwareAVLTree<String, KeyWrapper<String>>>> trees = new ArrayList<>();
	...
	public void add(final T rec) {
		...
		for (final Tuple<KeyWrapperFactory<T, String>, SimilarityAwareAVLTree<String, KeyWrapper<String>>> tuple : trees) {
			final KeyWrapperFactory<T, String> factory = tuple.getLeft();
			final SimilarityAwareAVLTree<String, KeyWrapper<String>> tree = tuple.getRight();
